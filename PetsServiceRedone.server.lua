--[[  Redid my script before resubmitting application. 
> I have now added checks for every time I use GetChildren() and FindFirstChild
> I have reorganized every last time I previously used nested functions, there are no longer any functions defined inside of other functions- organization is cleaner.
Ty so much for the time, whoever ends up reviewing this
...
function handleAttackSceneJumping(player,indicator,petObject,centerPoint)
			...
			for i = 1, numBounces do -- Iterate until the pet has bounced the numBounces amount.
				...
				if hitbox then -- Don't continue with hit detection if, for some reason, it isn't found
...
function petAttackScene(player, centerPoint, petFolder, indicator) -- This is the visual attack, throughout this the pets jump on the center and deal damage to anything in the circle- ensuring they keep circle formation as well as return to it.
	...
	local checkForAttackingParticle = indicator:FindFirstChild("AttackingParticle", true)
	if checkForAttackingParticle then checkForAttackingParticle.Enabled = true end -- This is for the shards particle: enable them with the scene.
	local activePets = petFolder:GetChildren() 
	if (not activePets) or #activePets == 0  then return end -- If the player has no folder, OR if they do not have any pets in said folder, do not continue.
	for _, petObject in pairs(activePets) do -- For every pet the player has spawned:
		...
		task.spawn(handleAttackSceneJumping,player,indicator,petObject,centerPoint) -- To avoid nesting and a super packed function, move on to handling jumping-on-target logic
	end
end
...
function createPetAttack(player, mouseHit)
	...
	if not petsFolder then petsFolder = Instance.new("Folder",workspace)
	petsFolder.Name = player.Name.."Pets" -- If FindFirstChild does not get the folder, create it.
end 
	local checkHasPets = petsFolder:GetChildren()
	if (not checkHasPets) then return end 
	local numPets = #checkHasPets-- Total # of pets the player currently has in their workspace folder
	if numPets == 0 then return end -- If no pets detected, cut it short here
	for i, petObject in pairs(checkHasPets) do 
		...
		task.spawn(attackSceneUponArrival,player,petObject,x_offset,z_offset,i,numPets,mouseHit,petsFolder,effect,waypoints) -- Once again avoid nesting by dividing up the logic in a readable way, this one handling telling the pets when to start jumping.

		if #waypoints > 0 then
			task.spawn(handleAttackCooldown,effect,petsFolder) -- Avoid nesting and stacking coroutines here, call to function to wait the cooldown time before next attack is doable
...
function callTheAttack(player)
	...
	if not character then return end -- End the attack before it begins if the player has no character
...
function initialMovementUpdate(player, petObject) -- This function is integral to replication of pet movement as will be described in the next lines
	sendMovementUpdate(player,petObject) -- The calling of this in the initialization of the function is to ensure that, even when they are very first spawned, they smoothly tween to the accurate position. More details on how this works in next lines.
	petObject:GetPropertyChangedSignal("CFrame"):Connect(function() sendMovementUpdate(player,petObject) end)
end 
...
function followingMoveDirChanged(player,clonedPetPart) -- Called whenever humanoid.MoveDirection is changed, more documentation on this connection at the bottom!
...
function followPath(player,clonedPetPart) -- Here is the function for having the pets consistently follow their designated player
			local checkCharacter = player.Character
			if not checkCharacter then return end -- Don't proceed if no character
			local checkHumanoid = checkCharacter:FindFirstChild("Humanoid")
			if not checkHumanoid then return end 
			if checkHumanoid.MoveDirection ~= Vector3.new(0, 0, 0) then -- If the player is currently moving when this is initialized,
				followingMoveDirChanged(player,clonedPetPart) -- Then let the pets know they should now start moving aswell
			end
			checkHumanoid:GetPropertyChangedSignal("MoveDirection"):Connect(function() followingMoveDirChanged(player,clonedPetPart) end) -- Same thing here, except this is an active connection to listen for all changes!
end
...
petClickedEvent.OnServerEvent:Connect(function(player, petName) -- Listener for the petClickedEvent: as described at the top, this is fired when a player tries to spawn a pet through their UI.
	...
	if not petsFolder then return print('r') end -- If this is missing it means there are no pet source models, which should not happen, but must be accounted for.
	local petPart = petsFolder:FindFirstChild(petName) -- From aforementioned source folder, get the matching pet template to be cloned for the server-side instance
	if petPart  then  -- Do not continue if the FindFirstChild failed
		local clonedPetPart = petPart:Clone() -- Server pet part (to be assigned to the passed player)
		local getPlayerPetsFolder = workspace:FindFirstChild(player.Name.."Pets")
		if not getPlayerPetsFolder then print('nopetfolder') return end -- If the folder is absent that means the player just hasnt loaded in yet, block them from spawning anything until they are initialized.
		local getPlayerPets = #(getPlayerPetsFolder:GetChildren())
		if not getPlayerPets then return end
		if getPlayerPets >= MAXPETS then return end -- Do not proceed if the player has the max pets or other dependency errors regarding the folder
		...
		clonedPetPart.Parent = getPlayerPetsFolder -- If all reqs were met, to the player's pet folder!
		...
		initialMovementUpdate(player, clonedPetPart) -- More documentation within the function, but it listens for the pets CFrame being changed to smoothly replicate the physics. 

			clonedPetPart:SetAttribute("Owner",player.Name)
		task.spawn(followPath,player,clonedPetPart)
	end
